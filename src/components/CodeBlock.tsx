...
const LANG_LABEL: Record<CodeBlockProps["language"], string> = {
  json: "JSON",
  bash: "BASH",
  javascript: "JS",
  text: "TEXT",
...
export function CodeBlock({ language, label, code, testid }: CodeBlockProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-zinc-900 bg-black/60 px-3 py-1.5">
        <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-300">
          {label ?? LANG_LABEL[language]}
        </span>
        <button
          type="button"
          onClick={onCopy}
      ...
      <pre
        className="overflow-x-auto px-3 py-2 text-[12px] leading-relaxed text-zinc-100"
